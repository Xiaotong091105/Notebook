...
Separately, the [Classifier idea](../Classifier/Classifier%20plan%20and%20rationale.md) is trying to build a router that picks the cheapest method (SQL, MedCAT, regex, or — rarely, since it costs money — an LLM) for a given request, based on evidence rather than guesswork. That router needs a benchmark: real past requests with a known correct answer to test each method against.
...
4. Turn the results into curated gold answers for the Classifier benchmark (stage 1 of the [Classifier plan](../Classifier/Classifier%20plan%20and%20rationale.md)).
